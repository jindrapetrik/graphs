...
public class MutableNode implements Linkable, MutableAttributed<MutableNode>, LinkTarget,
    ...
    private String getLinkFrom(Link link) {
        return ((MutableNodePoint) link.from).node.label.toString();
    }

    //JPEXS 
    private String getLinkTo(Link link) {
        return ((MutableNodePoint) link.to).node.label.toString();
    }

    //JPEXS
    private boolean linkEquals(Link a, Link b) {
        return Objects.equals(getLinkFrom(a), getLinkFrom(b)) && Objects.equals(getLinkTo(a), getLinkTo(b));
    }

    //JPEXS:
    public void removeLink(Link link) {
        for (int i = 0; i < links.size(); i++) {
            if (linkEquals(link, links.get(i))) {
                links.remove(i);
                i--;
            }
        }
